...
This project explores how to serve many LoRA adapters (specialist models) on a single GPU without hitting the "VRAM wall" or slowing to a crawl. We use a **simulation** to benchmark our approach: reproducible traces, a two-tier memory manager, a grouped scheduler, and a simulated execution engine.
...
We do **not** run real LLM inference. Instead we simulate the **lifecycle of a request** and the **management of adapter weights** so we can measure throughput, latency, and VRAM usage in a reproducible way. The simulation has four main blocks.
...
Install with: `pip install torch` (or your project’s full `requirements.txt` when added).
